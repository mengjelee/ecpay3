...
def register(request):
    if request.method == "POST":
        ...
        password = request.POST.get('password')
        user = User.objects.create(name = name, password = password, email = email, status = status, bank_account = bank_account, bank_no = bank_no)
...
def new_course(request):
    if request.method == 'GET':
        ...
        if User.objects.filter(account = student_id).exists():
            ...
            course = Class.objects.create(tutor = teacher_id, student = student_id, subject = subject, 
